      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {featuredPost && (
          <section className="mb-20">
            <div className="group relative overflow-hidden rounded-3xl bg-slate-100 aspect-[16/9] sm:aspect-[21/9]">
              ...
              <div className="absolute bottom-0 left-0 p-8 sm:p-12 w-full">
                ...
                <Link href={`/blog/${featuredPost.slug}`}>
                  <h2 className="text-3xl sm:text-5xl font-extrabold text-white hover:text-blue-200 transition-colors leading-tight mb-4">
                    {featuredPost.title}
                  </h2>
                </Link>
                <p className="text-slate-300 text-lg max-w-2xl line-clamp-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16">
            {remainingPosts.map((post) => (
              <article key={post.slug} className="group">
                <div className="mb-4 aspect-video bg-slate-50 rounded-2xl overflow-hidden border border-slate-100 shadow-sm group-hover:shadow-md transition-shadow">
                  {/* 이미지 렌더링 영역 (필요 시 추가) */}
                ...
                <div className="space-y-3">
                  <p className="text-xs font-bold text-blue-600 uppercase tracking-widest">Knowledge</p>
